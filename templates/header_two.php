		...
		if( 'on' === $top_header_choice ) {
			...
			<div class="bg-dark py-2">
				...
				<a class="navbar-brand m-0 d-flex align-items-center gap-2" href="<?= esc_url(home_url('/')); ?>">
					<?php the_custom_logo(); ?>
					<span class="text-cream small d-none d-md-inline">Restaurants • Bars • Music Venues — Uptown Columbus</span>
				</a>
				...
				<nav class="brandbar-nav d-none d-md-flex gap-2">
					<?php
						...
						foreach ( $subs as $label => $url ) {
							echo '<a class="btn btn-sm btn-outline-light rounded-pill px-3" href="' . esc_url( $url ) . '">' . esc_html( $label ) . '</a>';
						}
					?>
				</nav>
				</div>
