...
<div class="gen2 gen2-schematic">
	...
	*/ ?>
	...
	<!-- ────────── SERVICES ────────── -->
	<?php
	// Section chrome (subtitle, title, intro) lives on this page so the
	// Homepage's "2 — Services Section" CMB2 metabox keeps the numbered
	// section sequence intact. The repeatable service cards still pull
	// from the Services page so both pages share one catalogue.
	...
	$svc_logo_video   = gen2_meta( 'gen2_services_logo_video_url', '' );
	$svc_source_id    = gen2_get_services_page_id();

	// Detect direct-file video URLs vs YouTube/Vimeo embed URLs (same
	// pattern as the case-study hero video in section 5).
	$svc_video_ext        = $svc_logo_video ? strtolower( (string) pathinfo( (string) parse_url( $svc_logo_video, PHP_URL_PATH ), PATHINFO_EXTENSION ) ) : '';
	$svc_video_is_direct  = in_array( $svc_video_ext, array( 'mp4', 'webm', 'ogv', 'ogg', 'mov', 'm4v' ), true );
	$svc_video_mime       = ( 'mov' === $svc_video_ext ) ? 'mp4' : $svc_video_ext;
	...
	$cards_fallback = array(
		...
		array(
			'card_code'        => 'S-EDF',
			'card_title'       => 'Electrical Design and Fabrication (UL508A/ETL)',
		...
		array(
			'card_code'        => 'S-HMI',
			'card_title'       => 'HMI SCADA databases and software',
	...
	$cod_schem_image  = gen2_meta( 'gen2_codesys_schem_image',   '' );
	...
	<section class="gen2-schem-codesys">
		...
		<div class="gen2-schem-codesys__inner">
			...
			<div class="gen2-schem-codesys__main">
				...
				<div>
					<?php if ( $cod_schem_image ) : ?>
						<img class="gen2-schem-codesys__schem-img" src="<?php echo esc_url( $cod_schem_image ); ?>" alt="<?php echo esc_attr( wp_strip_all_tags( $cod_schem_label ) ); ?>" />
					<?php else : ?>
	...
	<?php
	// Section chrome (subtitle, title, intro) lives on this page so the
	// Homepage's "8 — Leadership Section" CMB2 metabox keeps its numbered
	// slot. The repeatable team members pull from the Staff page so both
	// pages share one roster.
